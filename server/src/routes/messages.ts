...
function toMessageJson(m: messageRepo.MessageRow) {
  return {
    ...
    quoted_image_url: m.quoted_image_url && m.quoted_message_id
      ? `/api/messages/${m.quoted_message_id}/media`
      : null,
  };
}
 ...
 */
router.get('/:id/media', authMiddleware, asyncHandler(async (req: Request, res: Response) => {
...
router.post('/', authMiddleware, upload.single('image'), validateBody(sendMessageSchema), asyncHandler(async (req: Request, res: Response) => {
  ...
  const imageFile = req.file
    ? path.basename(await compressImage(path.join(PATHS.uploadsPrivate, req.file.filename), { maxWidth: 1280 }))
    : null;
