...
export function markAllRead(userId: number): void {
  stmt('UPDATE messages SET read = 1 WHERE receiver_id = ? AND read = 0').run(userId);
}
...
export function listMessageHistory(
  ...
  // 自动标记对方发来的消息为已读
  stmt('UPDATE messages SET read = 1 WHERE sender_id = ? AND receiver_id = ?').run(
    otherUserId,
    currentUserId
  );
...
export function getMessageMedia(
  ...
  return stmt('SELECT id, sender_id, receiver_id, image_url FROM messages WHERE id = ?').get(messageId) as
    { id: number; sender_id: number; receiver_id: number; image_url: string | null } | undefined;
}
