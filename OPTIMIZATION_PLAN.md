...
**总体验收清单**（2026-09-11 收尾逐条勾核，括号内为证据所在章节）：

- [x] `npm run format:check && npm run lint && npm run build && npm test` 全绿 —— lint **0 error**/55 warning（16.6、18.1）
- [x] `npx tsc -p server/tsconfig.test.json --noEmit` 与新增的 e2e typecheck 全绿 —— 已并入根 `npm run typecheck`，CI 跑的就是这条（18.1）
- [x] `npm run e2e` 连跑 3 次无 flake —— **3×10 条全过**，退出码 `0,0,0`（18.6）
- [x] 单测用例数 ≥ 340 —— 服务端 242 + 客户端 353 = **595**（16.6）
- [x] 首屏 JS 从 530,690 B 下降（目标 < 480 KB）—— **463.3 KB**（13.1，线上核验见 18.4）
- [x] `client/dist` 体积下降 ≥ 1.6 MB（无用字体）—— 删掉的两份 `public/MiSans-*.woff2` 合计 **1,614,920 B**，索引内只剩被 `global.css` 引用的子集字体（17.4）
- [x] `server/uploads*` 测试后无新增孤儿文件 —— 跑完 3 轮 e2e 后 `server/uploads`、`uploads_private` 新增 **0** 个，真实库 mtime 仍停在 09-10（18.6）
- [x] 故意破坏一次部署，确认可回滚且站点不中断 —— 回滚 506ms / 前滚 509ms，旧代码兼容新库；「不中断」实测为约 **0.5s 的 502 窗口**（17.1）
- [x] 手测：进房瞬间退出 → 麦克风指示灯灭；双击点赞 → 计数正确 —— 已自动化替代（轨道 `ended` + 无残留定时器、双击只发一次 POST，两者都反向验证过）；**真机上那盏指示灯**仍需你扫一眼（17.2/17.3，15.4 第 9 项）
- [x] UI 与交互的**逐像素对比**：改动前后各截一轮主要页面 —— 4 个游客可见页面（首页/发现/书架/语音）**信号/噪声恒为 1.00**；私信/后台/阅读器需登录态未截，局限已写明（17.5，15.4 第 10 项）
...
- **UI 受阻**：~~管理端用户列表的服务端搜索/分页（现在客户端搜索只覆盖封顶的 500 行）~~ ——
  **已在第 19 章做完**（你确认后按「只增不改」加的契约：老客户端形状保留）；
...
**已在 22.1 做完**（sha256 一致即打印 `[APK] SKIP`）；
Dockerfile 运行时实测（本机无 Docker）；访客 id 多实例化（需 Redis）；
真机回归（低端安卓/弱网/麦克风指示灯）。
（~~管理端用户列表的服务端搜索与分页~~ 已在第 19 章完成；帖子管理 tab 的同类问题见 19.6。）

### 18.6 收尾复排：把第五章的验收清单本身也勾掉（2026-09-11 傍晚）

15.4 已经逐条核对过验收清单，但**第五章那十个复选框一直是空的** —— 文档读者会以为十项都没验。
本次把它们逐条勾上并附证据章节（见第五章）。复排中另补到两处：

- **「被忽略的文件」这一层此前没扫到**：仓库根目录多出一个 `__pycache__/`（`deploy-sftp.py` /
  `deploy-client-lite.py` 的运行产物，2 个 `.pyc`，13:47 生成）。18.5 只扫了工作区，因此漏掉了它 ——
  已删除。现在 `git status --porcelain --ignored` 在本仓库只剩：`.env`、`client/android/*`
  （keystore、构建产物、本地配置）、`server/k.db*` 与两份**刻意保留**的线上库快照、`server/uploads*`、
  `release/`、`e2e/.tmp`、`.workbuddy/` —— 全是预期内的本地状态，无临时脚本、无调试日志。
- **构建确定性再确认**：本轮 3 次 e2e 每次都重新跑了一遍 `npm run build`，产物仍是 `index-DevxUuZN.js`
  （125,778 B），与线上同名同尺寸；服务端 `dist` 的 sha256 亦与线上一致（18.4）。

**复排结论**：验收清单 **10/10** 勾上（第 9 项的真机指示灯、第 10 项的登录态页面截图，剩余部分需要你或
登录态环境配合，已在 15.4 标注为 🟡）；问题清单与批次项**无新增遗漏**；
`git ls-files` 中 >500 KB 的文件只剩三份必需资产（rnnoise 生成码 1.89 MB + 两份被引用的子集字体），
无「无用二进制随部署分发」。

---

## 19. 管理端用户列表：服务端搜索 + 分页（2026-09-11 晚，你点名先做的一项）

### 19.1 为什么这不是「加个分页控件」而是一次契约变更

老实现：`GET /api/admin/users` 一次返回最多 `HARD_LIST_CAP`(500) 行，**每行带一个相关 `COUNT(*)`**
（帖子数），客户端把整个列表渲染进 DOM 并在**本地**过滤。后果不是慢，而是**管不了**：
搜索框里搜不到第 501 个用户 → 他的封禁/改密/删除按钮根本不存在。
（生产当前 9 个用户，离上限很远 —— 属于「等它发生就晚了」的那一类。）

而线上有已安装的 APK 在跑老客户端，所以这次按 `listLimits.ts` 的同一条原则办：**只增不改**。

| 请求                                  | 响应                                                   | 谁在用                     |
| ------------------------------------- | ------------------------------------------------------ | -------------------------- |
| `GET /api/admin/users`（不带 page/q）  | `{ users, has_more }`（上限 500 行，形状与字段逐字不变） | 已安装的 APK（本地过滤）   |
| `GET /api/admin/users?page=&limit=&q=` | `{ users, total, page, limit, totalPages, has_more }`   | 网页端（服务端搜索 + 分页） |

### 19.2 服务端

- `shared/src/schemas/common.ts` 新增 `searchQuerySchema`：非法输入（数组/对象）退化成空串 =
  不过滤而不是 400，超长**截断**到 50 字符 —— 搜索框里的内容不该让整个列表报错。
- `repositories/admin.repo.ts` 新增 `countUsers(q)` 与 `listUsersPage({ q, page, limit })`。
  搜索条件刻意与客户端**原来的本地过滤**逐字段对齐：`username LIKE` / `email LIKE` /
  `CAST(id AS TEXT) LIKE`（三者是**或**关系），且统一走 `escapeLike` + `ESCAPE '\'` ——
  否则用户名里的 `%`/`_` 会变成通配符，搜 `a_b` 会命中 `axb`。
  `listUsers(cap)` 保留给老客户端，注释里写明了「为什么不能删」。
- `routes/admin/users.routes.ts`：`page` 或 `q` 出现即进分页模式，否则原样走老分支；
  `limit` 复用 `limitQuerySchema`（上限 50）。单次只物化 `limit` 行而不是 501 行。

### 19.3 客户端

- `AdminPage`：query key 变为 `['admin','users', page, q]`，`q` 走 300ms 防抖，
  翻页/改词用 `keepPreviousData` 保留旧页显示（与帖子管理一致）。
  「输入就把页码重置回第 1 页」写在 **onChange 处理函数**里而不是 `useEffect` 里 ——
  写在 effect 里会踩 `react-hooks/set-state-in-effect`（这条规则此前已经咬过一次）。
- `pages/admin/usersPaging.ts`（新）：把两处「错了也不报错」的算法抽成纯函数 ——
  `pageCount`（0 行 → 0 页、恰好整页不虚增）与 `removeUserFromPage` / `patchUserInPage`
  （乐观删除要同时修 `total`/`totalPages`，否则第二页删空了页脚还说有下一页；
  删一个**不在当前页**的 id 则原样返回，不能顺手把总数减掉）。
- `AdminUsersTab`：删掉本地 `filter`；分页控件复用帖子管理同款 `styles.pagination`
  （**没有新增任何 CSS**，视觉与既有后台一致）；补空态「没有匹配的用户 / 暂无用户」；
  补 `data-testid`（沿用 16.5 的约定：选择器不依赖 CSS Modules 哈希类名）。

### 19.4 验证（三层 + 反向验证）

- 服务端 **16** 条（`server/test/admin-users-pagination.test.ts`，走真实 `createApp()` + `fetch`）：
  分页窗口 / `total` / `totalPages` / `has_more`、两页 id 不重叠、越界页返回空、用户名·邮箱·ID
  子串搜索、搜索叠加分页、无命中、**通配符转义两条**、脏参数（`page=abc`、`limit=1000`、`q` 传数组、
  超长关键词）都不 500，以及 ★「不带 page/q 时响应里**不许出现** total/totalPages」。
- 客户端 **21** 条（10 条纯函数 + 11 条 Tab 渲染）：★「搜索词不匹配时行照样全部渲染」
  （本地过滤一旦被加回来就红）、分页控件边界（首页禁上一页 / 末页禁下一页 / 只有一页不渲染）、
  空态文案、翻页回调语义。
- E2E **1** 条（`e2e/admin-users.spec.ts`）：直连 e2e 库种 30 个用户 + 真实注册接口拿 token +
  `UPDATE users SET role='admin'`（**全程不 mock**，客户端 `/auth/me` 拿到的就是真管理员），
  然后走一遍：第一页 20 行且请求 URL 带 `limit=20` → 翻到第二页（响应里能看到目标用户）→
  输入**只在第二页出现**的用户名 → 断言服务端 `total=1` 且界面上只剩那一行 → 清空搜索回到 20 行。
  这条的意义在于两侧单测都是各自造假数据：参数名写成 `keyword`、或响应字段写成 `total_pages`，
  单测全绿而界面上是空表格 —— e2e 是唯一能挡住这类接缝错误的一层。
- **反向验证**：把服务端搜索临时改成忽略关键词（`const q = ''`），e2e 立刻红在
  `expect(foundBody.total).toBe(1)`，实测 `Received: 31`（= 全部用户）；改回后 11/11 绿。
  期间还纠了一个**测试自身的**错误断言：搜索是三字段的「或」，原断言要求「每行 id 都含 1」太窄
  —— 那是断言写错而不是产品错，已改成「三字段至少命中其一」+「用只能靠 ID 命中的管理员行
  （id=1，用户名/邮箱都不含 1）证明 ID 分支确实生效」。

### 19.5 验收

`format:check` / `typecheck` / `lint`（0 error）/ `build` 全绿；单测 **服务端 34 文件 258 用例、
客户端 40 文件 374 用例**（此前 33/242、38/353）；e2e **11/11**（此前 10）。README 的计数已同步。
...
### 22.1 APK 上传改成「sha256 一致就 SKIP」（2026-09-11 20:20，18.5 那条待优化）

**改法**（三处）：

- `deploy.ps1`：定位到 Gradle 产物后算本地 sha256（并打印前 12 位），经 `--apk-sha` 传给后端；
- `deploy-sftp.py`：上传前先 `sha256sum` **远端同名文件**，一致则打印
  `[APK] SKIP：跳过上传与旧包清理（本次省下约 15.5 MB 传输）`；不一致/远端没有该文件时照旧上传。
  比的是**内容**不是大小或时间戳 —— 同样大小的不同构建很常见，只比大小会漏掉新包；
- 部署后那条 APK 核验从「比大小」升级为「**比 sha256**」（给了 `--apk-sha` 时）。

**实测**（真跑了一次完整部署）：`找到新版 APK: k-app-0.2.38-release.apk (15.5 MB, sha256 2d31dbab76fc…)` →
`远端同名文件 sha256 一致 → [APK] SKIP` → 部署后核验 `2d31dbab…` 一致 → `[DEPLOY_VERIFY] PASS`，全程 2 分 21 秒。

**一个必须说清的点（不然就是夸大）**：这条省下的是**那次多余的单独上传**，不是「APK 完全不再过网」。
因为远端的第 ③ 步是把整个 `client/dist` **整体换掉**（旧的移进备份），所以真正把 APK 放到
`client/dist/apk/` 的是**部署包里的那份拷贝**；单独的 SFTP 上传本来就是重复的第二遍。
改前每个部署 APK 过网 **两遍**，改后**一遍**。要让 APK 在没发版时一遍都不过网，需要
「打包前先探测远端 sha + 远端在切换 dist 时先把 `apk/` 目录挪到一边再挪回来」——
动的是线上原子替换的核心步骤，收益（每次部署省十几 MB）与风险（挪不回来就是 APK 链接 404，
虽然新的 sha 核验会红）都由你定，**这次没做**。

**顺带查实两件事**：远端 `client/dist/apk/` 里其实**永远只有 1 个文件**（每次部署整体换 dist，
旧 APK 跟着旧 dist 进了备份），所以「保留最近 5 个」这条清理基本不会触发；
而 `.deploy-backup` 保留最近 3 份，因此历史 APK 最多留 3 份（约 46 MB），不会无限涨。
